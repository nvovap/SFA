...
class SalesFromBonuseViewTable: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "NewSales" {
            
            (segue.destinationViewController as! OutletsViewController).noRessidue = false
            
        } else if segue.identifier == "SelectSales" {
            if let indexPath = self.tableView.indexPathForSelectedRow {
                if let sales  = self.fetchedResultsController.objectAtIndexPath(indexPath) as? NVPSalesForBonus {
    ...
    @IBAction func saveSales(segue: UIStoryboardSegue){
        do {
    ...
    func configureCell(cell: UITableViewCell, atIndexPath indexPath: NSIndexPath) {
        if let object = self.fetchedResultsController.objectAtIndexPath(indexPath) as? NVPSalesForBonus {
            ...
            let text =  NSMutableAttributedString(string: pream + object.store.name)
    ...
    var fetchedResultsController: NSFetchedResultsController {
        ...
        let entity = NSEntityDescription.entityForName("SalesForBonus", inManagedObjectContext: self.managedObjectContext!)
