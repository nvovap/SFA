...
class TableResidue: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "SelectInventory" {
            if let indexPath = self.tableView.indexPathForSelectedRow {
                if let inventory = self.fetchedResultsController.objectAtIndexPath(indexPath) as? NVPInventory {
                    (segue.destinationViewController as! EditTableResidue).inventory = inventory
                }
            }
        }
    ...
    @IBAction func saveInventory(segue: UIStoryboardSegue){
        do {
    ...
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        if editingStyle == .Delete {
            ...
            do {
                ...
                // Replace this implementation with code to handle the error appropriately.
                // abort() causes the application to generate a crash log and terminate. You should not use this function in a shipping application, although it may be useful during development.
                //println("Unresolved error \(error), \(error.userInfo)")abort()
            }
        }
    }
    
    func configureCell(cell: UITableViewCell, atIndexPath indexPath: NSIndexPath) {
        if let object = self.fetchedResultsController.objectAtIndexPath(indexPath) as? NVPInventory {
            // println(object.store.name)
            ...
            let text =  NSMutableAttributedString(string: pream + object.story.name)
    ...
    var fetchedResultsController: NSFetchedResultsController {
        ...
        let entity = NSEntityDescription.entityForName("Inventory", inManagedObjectContext: self.managedObjectContext!)
       ...
       // var predicate = NSPredicate(format: "free == NO")
